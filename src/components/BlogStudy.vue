<template>
  <div id="articleContent">
    <ul>
      <li class="articleItem" v-for="article in articles" v-bind:todo="article" v-bind:key="article.id" @click="toArticle(article)">
    ...
    <loading-study v-if="loading"></loading-study>
  </div>
</template>
<script>
import { store } from '../store/store'
import axios from 'axios'
import LoadingStudy from './LoadingStudy.vue'

export default {
  store: store,
  name: 'BlogArticle',
  components: {
    'loading-study': LoadingStudy
  },
  data () {
    return {
      articles: [],
  ...
  methods: {
    getArticles: function () {
      this.loading = true
      axios.get('https://api.github.com/repos/MichaelMa666/MichaelMa666.github.io/issues?labels=study', {header: {'Authorization': 'token ' + this.$store.state.accessToken}})
        .then((response) => {
          this.loading = false
